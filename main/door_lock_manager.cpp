...
bool BoltLockManager::IsValidCredentialIndex(uint16_t credentialIndex, CredentialTypeEnum type)
{
    ...
    if (CredentialTypeEnum::kPin != type) {
        return false;
    }
...
CHIP_ERROR BoltLockManager::InitLockState()
{
    ...
    if (!DoorLockServer::Instance().GetNumberOfCredentialsSupportedPerUser(endpointId, numberOfCredentialsPerUser)) {
        ESP_LOGE(TAG, "Unable to get credentials-per-user limit; defaulting to 3 [endpointId=%d]", endpointId);
        numberOfCredentialsPerUser = 3;
    }

    uint16_t numberOfUsers = 0;
    if (!DoorLockServer::Instance().GetNumberOfUserSupported(endpointId, numberOfUsers)) {
        ESP_LOGE(TAG, "Unable to get supported-user limit; defaulting to 20 [endpointId=%d]", endpointId);
        numberOfUsers = 20;
    }
